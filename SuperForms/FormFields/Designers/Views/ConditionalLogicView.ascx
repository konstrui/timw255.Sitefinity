...
<div id="logicView">
    <input type="checkbox" id="usesConditionalLogic" /> <%$Resources:LogicalFormsResources, UseConditionalLogicLabel %>
    <div id="conditionalLogic" style="display:none;">
        <div>
            <select id="action">
                <option value="0"><%$Resources:LogicalFormsResources, ShowActionOptionText %></option>
                <option value="1"><%$Resources:LogicalFormsResources, HideActionOptionText %></option>
            </select>
            <%$Resources:LogicalFormsResources, IfLabelText %>
            <select id="quantity">
                <option value="0"><%$Resources:LogicalFormsResources, AnyBoolOptionText %></option>
                <option value="1"><%$Resources:LogicalFormsResources, AllBoolOptionText %></option>
            </select>
            <%$Resources:LogicalFormsResources, OfTheFollowingCriteriaMatch %>
        </div>
        ...
        <div id="criteriaList">
            ...
            <script type="text/x-kendo-tmpl" id="criteriaTemplate">
                <tr>
                    ...
                    <td class="delete"><a class="deleteCriteria sfDeleteBtn" data-id="#: id #" title="<%$Resources:LogicalFormsResources, RemoveButtonText %>"></a></td>
                </tr>
            </script>
        </div>
        ...
        <div id="criteriaBuilder">
            ...
            <a class="sfLinkBtn" id="addCriteria"><strong class="sfLinkBtnIn"><%$Resources:LogicalFormsResources, AddButtonText %></strong></a>
        </div>
    </div>
</div>
...
<script>
        var criteriaOptionsDataSource = new kendo.data.DataSource({
            ...
            filter: { field: "fieldName", operator: "neq", value: optionFilter }
        });
        ...
        $("#addCriteria").click(function (e) {
            ...
            newCriteria.fieldName = $("#field").val();
